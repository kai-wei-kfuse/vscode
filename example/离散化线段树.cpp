...
void update_tree(int node, int l, int r, int ql, int qr, int k) {
    ...
    int mid = (l + r) >> 1;
    if (ql <= mid)
        update_tree(node << 1, l, mid, ql, qr, k);
    if (qr > mid)
        update_tree(node << 1 | 1, mid + 1, r, ql, qr, k);
}
void query_tree(int node, int l, int r, int ql, int qr) {
    ...
    int mid = (l + r) >> 1;
    if (ql <= mid)
        query_tree(node << 1, l, mid, ql, qr);
    if (qr > mid)
        query_tree(node << 1 | 1, mid + 1, r, ql, qr);
}
int main() {
    int c;
    scanf("%d", &c);
