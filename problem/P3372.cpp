...
void update_tree(ll node,ll l,ll r,ll xl,ll xr,ll k)
{
    ...
    ll mid=(l+r)>>1;//(l+r)/2
    push_down(node,l,r);
    if(xl<=mid) update_tree(left_node,l,mid,xl,xr,k);
    if(xr>mid) update_tree(right_node,mid+1,r,xl,xr,k);
    tree[node]=tree[left_node]+tree[right_node];
}
ll query_tree(ll node,ll l,ll r,ll ql,ll qr){
    ...
    push_down(node,l,r);
    ll res=0;
    if(ql<=mid)res+=query_tree(left_node,l,mid,ql,qr);
    if(qr>mid)res+=query_tree(right_node,mid+1,r,ql,qr);
    return res;
}
int main()
{
    ...
    while(m--){
        ...
        if(t==1){
            ...
            update_tree(1,1,n,x,y,z);//(ll node,ll l,ll r,ll xl,ll xr,ll k)
        }
        else if(t==2){
            scanf("%lld %lld",&x,&y);
