...
void RoundRobin(Word* wait_queue, Word* run_queue, Word* finish_queue, FreeSpace* space);

void show(FreeSpace* space, Word* wait_queue, Word* finish_queue, Word* run_queue);
...
void show(FreeSpace* space, Word* wait_queue, Word* finish_queue, Word* run_queue) {
    ...
    while (p != NULL) {
        printf("%d\t\t%d\t\t%d\t\t%d\t\t%d \n", p->num, p->start, p->size, p->state, p->wordname);
        p = p->next;
    }
    printf("\n");
    printf("作业运行情况：\n");
    ...
    while (w != NULL)
    {
        printf("%d\t\t%d\t\t%d\t\t%c\n", w->wordname, w->run_time, w->size, w->wordstate);
        w = w->next;
    }
    printf("运行队列\n");
    if (r == NULL) {
    ...
    while (r != NULL)
    {
        printf("%d\t\t%d\t\t%d\t\t%c\n", r->wordname, r->run_time, r->size, r->wordstate);
        r = r->next;
    }
    printf("完成队列\n");
    if (f == NULL) {
    ...
    while (f != NULL)
    {
        printf("%d\t\t%d\t\t%d\t\t%c\n", f->wordname, f->run_time, f->size, f->wordstate);
        f = f->next;
    }
    printf("\n");
    system("pause");
...
void RoundRobin(Word* wait_queue, Word* run_queue, Word* finish_queue, FreeSpace* space) {
    ...
    show(space, wait_queue, finish_queue, run_queue);
     while ((wait_queue->next != NULL || run_queue->next != NULL) && number <= 5) {
