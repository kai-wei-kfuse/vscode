...
void RoundRobin(process* wair_queue,process*block_queue,process* run_queue,process*finish_queue);
...
void print(process* run_queue, process* finish_queue,process*block_queue) {
	...
	{
		printf(" 资源名称\t最大需求资源\t需求资源\t已分配资源\t进程运行时间\t进程状态\t剩余资源\n");
	...
	while (r!=NULL)
	{
		...
		for (int i = 0; i < Resources_number; i++) {
			if(i==0)
				...
				r->Need[i], r->Allocate[i], r->run_time, r->Process_state,Available[i]);
			else
			{
				printf("%4c\t\t%4d\t\t%4d\t\t%4d\t\t%4d\t\t%4c\t\t%4d\n", Name[i], r->Max[i],
					r->Need[i], r->Allocate[i], r->run_time, r->Process_state,Available[i]);
	...
	while ( b!= NULL)
	{
		...
		for (int i = 0; i < Resources_number; i++) {
			if (i == 0)
				printf("%4c\t\t%4d\t\t%4d\t\t%4d\t\t%4d\t\t%4c\t\t%4d\n", Name[i], b->Max[i], b->Need[i], b->Allocate[i], b->run_time,b->Process_state,Available[i]);
			else
			{
				printf("%4c\t\t%4d\t\t%4d\t\t%4d\t\t%4d\t\t%4c\t\t%4d\n", Name[i], b->Max[i], b->Need[i], b->Allocate[i], b->run_time, b->Process_state,Available[i]);
			}
	...
	while (f != NULL)
	{
		...
		for (int i = 0; i < Resources_number; i++) {
			...
			{
				printf("%4c\t\t%4d\t\t%4d\t\t%4d\t\t%4d\t\t%4c\t\t%4d\n",
				 Name[i], f->Max[i], f->Need[i], f->Allocate[i], f->run_time, f->Process_state,Available[i]);
			}
			else
			{
				printf("%4c\t\t%4d\t\t%4d\t\t%4d\t\t%4d\t\t%4c\t\t%4d\n",
...
void RoundRobin(process*wair_queue,process*block_queue,process* run_queue,process*finish_queue) {
	...
	while ((wair_queue->next!=NULL||block_queue->next!=NULL||run_queue->next!=NULL)&&number<=Process_run_number) {
...
int main() {
	...
	srand((int)time(NULL));
	if (wair_queue == NULL&&run_queue==NULL&&block_queue==NULL&&finish_queue) {
		printf("动态分配内存失败");
	}
